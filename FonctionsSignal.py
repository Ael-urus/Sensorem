...
def seuil_capteur2():
    ...
    sensibilité = 0.15 seuil de nettoyage, valeur de bruit du signal, permet d'éliminer les valeurs
                    qui peuvent polluer le signal (delta entre deux valeurs)
...
def sep_values(sv,seuil_capt):
    ...
    for i in range(nb_values - abs(nb_remplacement) - 9):
        if abs(sv[i + 9] - sv[i]) < sensibilite:  # attention le i+x (9) est à mettre en accord avec les valeurs d'exclusion des moyenne et ecartype du pdf et de l'interface graphique
            sv[i] = sv[i]
        else:
...
def make_paliers(paliers_find, plage_len_find):
    """Création d'une liste avec chaque palier, le passage est obscur sur le pourquoi :)
    je refais une liste imbriquée avec le nombre de paliers associer à ses valeurs de paliers
...
def isol_capteurs(values):
    ...
    tout ce qui suit jusqu'a l'autre nom ou la fin sont les données du capteur :p
    TBC
    """
    del values[0:23]
    last_key = None
...
def traitement_general_donnees(paliers_find, paliers_find2, values_sep_paliers, values_sep_paliers2, entete):
    ...
    if len(range(paliers_find)) == len(range(paliers_find2)):
        ...
        for i in range(paliers_find):
            moyenne[i] = mean(values_sep_paliers[i][
                              18: -18])  # Correction, supression des valeurs de début et de fin pour les traitements.
            ecartype[i] = pstdev(values_sep_paliers[i][18: -18]) * 1000
